...
Resource Fonts included in this package, they are avail*scable in the
[[/fonts][ ~fonts~ ]] directory.

There is a function which will download the latest fonts for this
package and install them in the /(guessed)/ appropriate directory
based on the OS. 
...
Bear in mind, this will also run ~fc-cache -f -v~ on MacOS and Linux
which /can take some time/ to complete.

For Windows, this function will prompt for a /download/ directory for
