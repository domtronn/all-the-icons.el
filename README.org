...
You can also override the /height/ ~:height~, /face/ ~:face~ and
/vertical adjustment/ ~:v-adjust~ of the icon when calling
~all-the-icons-icon-for-file~ or ~all-the-icons-icon-for-mode~ by
calling them with these properties, /e.g./
